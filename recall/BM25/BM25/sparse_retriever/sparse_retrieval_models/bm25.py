...
from ...utils.numba_utils import (
    intersect_sorted, #将多个有序数组合并，只保留交集
    intersect_sorted_multi, 
    union_sorted_multi,#将多个有序数组合并，保留并集
    unsorted_top_k, #从无序数组中找到topk的数值和对应的索引
)

"""
返回topk文档的docid和对应的分数
"""
@njit(cache=True)
def bm25(
    b: float,
    k1: float,
    term_doc_freqs: nb.typed.List[np.ndarray], #一个term出现在doc_ids的文档中的出现次数
    doc_ids: nb.typed.List[np.ndarray],#和上面变量对应
